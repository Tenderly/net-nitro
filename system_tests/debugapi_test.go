...
import (
	...
	"github.com/tenderly/net-nitro/go-ethereum/eth/gasestimator"
...
func TestArbTxTypesTracingPrestateTracerAndCallTracer(t *testing.T) {
	...
	if l2Tx.SkipNonceChecks() && result.Post[faucetAddr].Nonce != nil {
...
type blockTraceResult struct {
	Result PrePostStateMap `json:"result"`
...
func AutomatedPrestateTracerTest(t *testing.T, client *TestClient) {
	...
	for block := uint64(2); block <= blockHeight; block++ {
		...
		// #nosec G115
		err = client.Client.Client().CallContext(client.ctx, &traces, "debug_traceBlockByNumber", rpc.BlockNumber(block), traceConfig)
		Require(t, err)

		for i, trace := range traces {
			if trace.Error != "" {
				Fatal(t, "Tracer returned error for tx ", trace.TxHash, ": ", trace.Error)
			}
			for addr, contents := range trace.Result.Pre {
				...
				// That just means zero nonce has been serialized to nil
				if contents.Nonce == nil && runningAccount.Nonce != 0 {
					Fatal(t, "Nonce of account ", addr, " was nil in prestate tracer")
				}

				if (contents.Nonce == nil && runningAccount.Nonce != 0) ||
					(contents.Nonce != nil && *contents.Nonce != runningAccount.Nonce) {
				...
				accountPostTrace, accountInPost := trace.Result.Post[addr]
			...
			for addr, contents := range trace.Result.Post {
				hashedAddr := crypto.Keccak256Hash(addr.Bytes())
				runningAccount, hadAccount := runningState.HashedAccounts[hashedAddr]
				preTrace, inPre := trace.Result.Pre[addr]
				...
				if contents.Balance != nil {
					...
					runningAccount.Balance = new(big.Int).SetUint64(0)
				}
