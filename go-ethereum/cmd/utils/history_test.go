...
import (
	...
	"github.com/tenderly/net-nitro/go-ethereum/notinternal/era"
...
func TestHistoryImportAndExport(t *testing.T) {
	...
	chain, err := core.NewBlockChain(db, nil, genesis, ethash.NewFaker(), nil)
	if err != nil {
	...
	imported, err := core.NewBlockChain(db2, nil, genesis, ethash.NewFaker(), nil)
	if err != nil {
		t.Fatalf("unable to initialize chain: %v", err)
	}
