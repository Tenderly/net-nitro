...
func TestGeneratePOSChain(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(db, nil, gspec, engine, nil)
...
func ExampleGenerateChain() {
	...
	// Import the chain. This runs all block validation rules.
	blockchain, _ := NewBlockChain(db, nil, gspec, ethash.NewFaker(), DefaultConfig().WithStateScheme(rawdb.HashScheme))
	defer blockchain.Stop()
