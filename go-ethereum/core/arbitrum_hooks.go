...
type TxFilterer interface {
	...
	Setup(statedb vm.StateDB)
	// TouchAddresses marks sender, recipient, aliased, and retryable addresses for filtering.
	TouchAddresses(statedb vm.StateDB, tx *types.Transaction, sender common.Address)
	...
	CheckFiltered(statedb vm.StateDB, rootTx *types.Transaction, header *types.Header) error
}
