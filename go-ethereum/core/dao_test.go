...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, nil, nil, progspec, nil, ethash.NewFaker(), vm.Config{}, nil)
	defer proBc.Stop()
	...
	conBc, _ := NewBlockChain(conDb, nil, nil, congspec, nil, ethash.NewFaker(), vm.Config{}, nil)
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		bc, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, nil, congspec, nil, ethash.NewFaker(), vm.Config{}, nil)
		...
		bc, _ = NewBlockChain(rawdb.NewMemoryDatabase(), nil, nil, progspec, nil, ethash.NewFaker(), vm.Config{}, nil)
