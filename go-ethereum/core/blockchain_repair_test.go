...
func testRepairWithScheme(t *testing.T, tt *rewindTest, snapshots bool, scheme string) {
	...
	chain, err := NewBlockChain(db, nil, gspec, engine, option)
	if err != nil {
	...
	newChain, err := NewBlockChain(db, nil, gspec, engine, option)
	if err != nil {
...
// TestIssue23496 tests scenario described in https://github.com/tenderly/net-nitro/go-ethereum/pull/23496#issuecomment-926393893
...
func testIssue23496(t *testing.T, scheme string) {
	...
	chain, err = NewBlockChain(db, nil, gspec, engine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	defer chain.Stop()
