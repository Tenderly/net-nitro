...
func (s *StateDB) StartPrefetcher(namespace string, witness *stateless.Witness, witnessStats *stateless.WitnessStats) {
	...
	// https://github.com/tenderly/net-nitro/go-ethereum/issues/29880
...
// CopyVMStateDB returns a deep copy as an `any` so the vm package can obtain an
// independent StateDB without a direct *state.StateDB return type.
func (s *StateDB) CopyVMStateDB() any {
	return s.Copy()
}
