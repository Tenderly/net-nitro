...
func opExtCodeCopyEIP4762(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	scope.Contract.UseMultiGas(multigas.StorageAccessGas(consumed), evm.Config.Tracer, tracing.GasChangeUnspecified)
	if consumed < wanted {
	...
	*pc += 1
	if *pc < codeLen {
		...
		if !scope.Contract.IsDeployment && !scope.Contract.IsSystemCall && *pc%31 == 0 {
			...
			scope.Contract.UseMultiGas(multigas.StorageAccessGas(wanted), evm.Config.Tracer, tracing.GasChangeUnspecified)
			if consumed < wanted {
				return nil, ErrOutOfGas
			}
		}
