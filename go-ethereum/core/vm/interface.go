...
type StateDB interface {
	...
	// SetBalance overrides the balance at addr to amount (used by eth_call overrides).
	SetBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason)
	// SetStorage replaces the contract storage at addr with the provided map.
	SetStorage(addr common.Address, storage map[common.Hash]common.Hash)
	// SetTxContext binds the StateDB to a specific transaction within a block.
...
// CopyStateDB returns an independent copy of s suitable for trial execution
// (e.g. binary-search gas estimation, access-list tracing). State package can't
// import vm, so the canonical Copy() return type stays *state.StateDB; this helper
// indirects through an any-returning CopyVMStateDB hook that implementations expose.
func CopyStateDB(s StateDB) StateDB {
	if c, ok := s.(interface{ CopyVMStateDB() any }); ok {
		if r, ok := c.CopyVMStateDB().(StateDB); ok {
			return r
		}
	}
	panic("vm.CopyStateDB: StateDB implementation doesn't support copy")
}
