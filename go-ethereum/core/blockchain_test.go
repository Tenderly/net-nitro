...
import (
	...
	"github.com/tenderly/net-nitro/go-ethereum/consensus/beacon"
	"github.com/tenderly/net-nitro/go-ethereum/consensus/ethash"
	"github.com/tenderly/net-nitro/go-ethereum/core/history"
	...
	"github.com/tenderly/net-nitro/go-ethereum/core/vm/program"
...
func newCanonical(engine consensus.Engine, n int, full bool, scheme string) (ethdb.Database, *Genesis, *BlockChain, error) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, genesis, engine, options)
...
func testFastVsFullChains(t *testing.T, scheme string) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec, ethash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, nil, gspec, ethash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, nil, gspec, ethash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer ancient.Stop()
...
func testLightVsFastVsFullChainHeads(t *testing.T, scheme string) {
	...
	archive, _ := NewBlockChain(archiveDb, nil, gspec, ethash.NewFaker(), options)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	light, _ := NewBlockChain(lightDb, nil, gspec, ethash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
...
func testChainTxReorgs(t *testing.T, scheme string) {
	...
	blockchain, _ := NewBlockChain(db, nil, gspec, ethash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
...
func testLogReorgs(t *testing.T, scheme string) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, ethash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer blockchain.Stop()
...
func testLogRebirth(t *testing.T, scheme string) {
	var (
		...
		blockchain, _ = NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, engine, DefaultConfig().WithStateScheme(scheme))
	)
	defer blockchain.Stop()
...
//
// https://github.com/tenderly/net-nitro/go-ethereum/pull/15941
...
func testBlockchainHeaderchainReorgConsistency(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, genesis, engine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
...
func TestTrieForkGC(t *testing.T) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, genesis, engine, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testLargeReorgTrieGC(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(db, nil, genesis, engine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
//
// Details at:
//   - https://github.com/tenderly/net-nitro/go-ethereum/issues/18977
//   - https://github.com/tenderly/net-nitro/go-ethereum/pull/18988
...
func testLowDiffLongChain(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(diskdb, nil, genesis, engine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, engine, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testInsertKnownChainData(t *testing.T, typ string, scheme string) {
	...
	chain, err := NewBlockChain(chaindb, nil, genesis, engine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testInsertKnownChainDataWithMerging(t *testing.T, typ string, mergeHeight int) {
	...
	chain, err := NewBlockChain(chaindb, nil, genesis, engine, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testDeleteCreateRevert(t *testing.T, scheme string) {
	...
	// Import the canonical chain
	options := DefaultConfig().WithStateScheme(scheme)
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, engine, options)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testSetCanonical(t *testing.T, scheme string) {
	...
	options := DefaultConfig().WithStateScheme(scheme)
	chain, err := NewBlockChain(diskdb, nil, gspec, engine, options)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testChainReorgSnapSync(t *testing.T, ancientLimit uint64) {
	...
	chain, _ := NewBlockChain(db, nil, gspec, beacon.New(ethash.NewFaker()), options)
	defer chain.Stop()
...
func testInsertChainWithCutoff(t *testing.T, cutoff uint64, ancientLimit uint64, genesis *Genesis, blocks []*types.Block, receipts []types.Receipts) {
	...
	options := DefaultConfig().WithStateScheme(rawdb.PathScheme)
	chain, _ := NewBlockChain(db, nil, genesis, beacon.New(ethash.NewFaker()), options)
	defer chain.Stop()
