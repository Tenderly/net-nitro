...
import (
	...
	"github.com/tenderly/net-nitro/go-ethereum/consensus/misc/eip1559"
	...
	"github.com/tenderly/net-nitro/go-ethereum/notinternal/ethapi/override"
...
func (sim *simulator) processBlock(ctx context.Context, block *simBlock, header, parent *types.Header, headers []*types.Header, timeout time.Duration) (*types.Block, []simCallResult, map[common.Hash]common.Address, error) {
	...
	tracingStateDB := sim.state
	if hooks := tracer.Hooks(); hooks != nil {
		concreteState, ok := sim.state.(*state.StateDB)
		if !ok {
			return nil, nil, nil, errors.New("simulate: tracer hooks require a concrete *state.StateDB backend")
		}
		tracingStateDB = state.NewHookedState(concreteState, hooks)
	}
	...
	for i, call := range block.Calls {
		...
		concreteSimState, _ := sim.state.(*state.StateDB)
		receipts[i] = core.MakeReceipt(evm, result, concreteSimState, blockContext.BlockNumber, common.Hash{}, blockContext.Time, tx, gasUsed, root)
	...
	concreteFinalizeState, ok := sim.state.(*state.StateDB)
	if !ok {
		return nil, nil, nil, errors.New("simulate: FinalizeAndAssemble requires a concrete *state.StateDB backend")
	}
	b, err := sim.b.Engine().FinalizeAndAssemble(chainHeadReader, header, concreteFinalizeState, blockBody, receipts)
	if err != nil {
		return nil, nil, nil, err
	}
...
func (sim *simulator) activePrecompiles(base *types.Header) vm.PrecompiledContracts {
	var (
		...
		rules   = sim.chainConfig.Rules(base.Number, isMerge, base.Time, types.DeserializeHeaderExtraInformation(base).ArbOSFormatVersion)
	)
