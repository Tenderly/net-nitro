...
func createRegisterAPIBackend(backend *Backend, filterConfig filters.Config, fallbackClientUrl string, fallbackClientTimeout time.Duration, archiveRedirects []BlockRedirectConfig, txFilterer core.TxFilterer) (*filters.FilterSystem, error) {
...
func (a *APIBackend) FeeHistory(
	...
	vmState, _, err := a.StateAndHeaderByNumber(ctx, newestBlock)
	if err != nil {
		return common.Big0, nil, nil, nil, nil, nil, err
	}
	concreteState, ok := vmState.(*state.StateDB)
	if !ok {
		return common.Big0, nil, nil, nil, nil, nil, errors.New("FeeHistory: backend returned non-state.StateDB")
	}
	speedLimit, err := core.GetArbOSSpeedLimitPerSecond(concreteState)
	if err != nil {
		return common.Big0, nil, nil, nil, nil, nil, err
	}
...
func (a *APIBackend) StateAndHeaderByNumber(ctx context.Context, number rpc.BlockNumber) (vm.StateDB, *types.Header, error) {
...
func (a *APIBackend) TxFilter() core.TxFilterer { return a.txFilterer }

func (a *APIBackend) Engine() consensus.Engine {
	return a.b.Engine()
}

func (a *APIBackend) Pending() (*types.Block, types.Receipts, vm.StateDB) {
	return nil, nil, nil
}
