...
import (
	...
	"github.com/tenderly/net-nitro/go-ethereum/params/forks"
...
func (api *ConsensusAPI) GetBlobsV2(hashes []common.Hash) ([]*engine.BlobAndProofV2, error) {
	head := api.eth.BlockChain().CurrentHeader()
	if api.config().LatestFork(head.Time, types.DeserializeHeaderExtraInformation(head).ArbOSFormatVersion) < forks.Osaka {
