...
type Options struct {
	...
	RunScheduledTxes func(context.Context, core.NodeInterfaceBackendAPI, vm.StateDB, *types.Header, vm.BlockContext, *core.MessageRunContext, *core.ExecutionResult, core.TxFilterer) (*core.ExecutionResult, error)
...
func Estimate(ctx context.Context, call *core.Message, opts *Options, gasCap uint64) (uint64, []byte, error) {
	...
	if hi > params.MaxTxGasRenamedForNitroMerges {
...
func run(ctx context.Context, call *core.Message, opts *Options) (*core.ExecutionResult, error) {
	...
	// state itself rather than opts.State.Copy(). The binary search calls this
	...
	// wrappers alike) and avoids requiring a Copy/CopyVMStateDB hook, which the
	// Tenderly Arbitrum Nitro StateDB wrapper does not implement.
