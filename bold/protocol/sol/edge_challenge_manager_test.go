...
func TestEdgeChallengeManager_IsUnrivaled(t *testing.T) {
	...
	leafAdder := func(stateManager state.Provider, leaf protocol.Assertion) protocol.VerifiedRoyalEdge {
...
func TestEdgeChallengeManager_HasLengthOneRival(t *testing.T) {
	...
	t.Run("post bisection, mutual edge is one step fork source", func(t *testing.T) {
		...
		for height > 1 {
			...
			require.NoError(t, err)
			evilProof, err := evilStateManager.PrefixProof(ctx, prefixCommitReq, state.Height(height/2))
			require.NoError(t, err)
...
func TestEdgeChallengeManager_AddSubchallengeLeaf(t *testing.T) {
	...
	for blockHeight > 1 {
		bisectTo := state.Height(blockHeight / 2)
		req := &state.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []state.Height{},
...
func TestEdgeChallengeManager_ConfirmByOneStepProof(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		...
		data, startInclusionProof, endInclusionProof, err := honestStateManager.OneStepProofData(
			...
			assertionMetadata,
			[]state.Height{
				state.Height(fromBlockChallengeHeight),
				state.Height(fromBigStep),
			},
			state.Height(smallStep),
		)
		require.NoError(t, err)
...
func TestEdgeChallengeManager_ConfirmByTime(t *testing.T) {
	...
	bisectTo := state.Height(challenge_testing.LevelZeroBlockEdgeHeight / 2)
	req := &state.HistoryCommitmentRequest{
		AssertionMetadata:           simpleAssertionMetadata(),
		UpperChallengeOriginHeights: []state.Height{},
		UpToHeight:                  containers.Some(bisectTo),
...
func setupBisectionScenario(
	...
	// Honest assertion being added.
	leafAdder := func(stateManager state.Provider, leaf protocol.Assertion) (history.History, protocol.VerifiedRoyalEdge) {
		req := &state.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []state.Height{},
			UpToHeight:                  containers.Some(state.Height(0)),
...
func setupOneStepProofScenario(
	...
	bigStepAdder := func(stateManager state.Provider, sourceEdge protocol.SpecEdge) protocol.VerifiedRoyalEdge {
		req := &state.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []state.Height{0},
			UpToHeight:                  containers.Some(state.Height(0)),
	...
	for bigStepHeight > 1 {
		bisectTo := state.Height(bigStepHeight / 2)
	...
	smallStepAdder := func(stateManager state.Provider, edge protocol.SpecEdge) protocol.VerifiedRoyalEdge {
		req := &state.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
	...
	for smallStepHeight > 1 {
		bisectTo := state.Height(smallStepHeight / 2)
		req := &state.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []state.Height{0, 0},
			UpToHeight:                  containers.Some(bisectTo),
