...
// Package tracker contains the logic for tracking an edge in the challenge manager. It keeps
...
type HonestChallengeTreeReader interface {
	...
	IsConfirmableEssentialEdge(
		...
		confirmationThreshold uint64,
	) (confirmable bool, essentialPaths []tree.EssentialPath, timer uint64, err error)
}
...
type Tracker struct {
	...
	associatedAssertionMetadata *state.AssociatedAssertionMetadata
...
func New(
	...
	chainWatcher HonestChallengeTreeWriter,
	challengeManager ChallengeTracker,
	assertionCreationInfo *state.AssociatedAssertionMetadata,
	opts ...Opt,
) (*Tracker, error) {
	tr := &Tracker{
		edge:                        edge,
...
func (et *Tracker) AssertionInfo() *state.AssociatedAssertionMetadata {
...
func (et *Tracker) tryToConfirmEdge(ctx context.Context) (bool, error) {
	...
	if err != nil {
		...
		if errors.Is(err, tree.ErrChildrenNotYetSeen) {
			return false, nil
		}
...
func (et *Tracker) DetermineBisectionHistoryWithProof(
	...
	challengeOriginHeights := make([]state.Height, len(originHeights.ChallengeOriginHeights))
	for index, height := range originHeights.ChallengeOriginHeights {
		challengeOriginHeights[index] = state.Height(height)
	}
...
func (et *Tracker) openSubchallenge(ctx context.Context) error {
	...
	fromBlockChallengeHeight := state.Height(originHeights.ChallengeOriginHeights[0])
	...
	default:
		heights := make([]state.Height, 0)
		for _, h := range originHeights.ChallengeOriginHeights {
			heights = append(heights, state.Height(h))
		}
		heights = append(heights, state.Height(startHeight))
