...
func (ht *RoyalChallengeTree) prepareHistoryCommitmentRequest(
...
) (*state.HistoryCommitmentRequest, error) {
	...
	assertionMetadata := &state.AssociatedAssertionMetadata{
		FromState:            fromState,
		BatchLimit:           state.Batch(parentCreationInfo.InboxMaxCount.Uint64()),
	...
	startHeights := make([]state.Height, len(heights.ChallengeOriginHeights))
	for i, h := range heights.ChallengeOriginHeights {
		startHeights[i] = state.Height(h)
	}
