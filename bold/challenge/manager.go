...
// Package challenge includes the main entrypoint for setting up a BoLD
...
type Manager struct {
	...
	trackedEdgeIds               *threadsafe.Map[protocol.EdgeId, *tracker.Tracker]
	assertionMetadataCache       *threadsafe.LruMap[protocol.AssertionHash, state.AssociatedAssertionMetadata]
...
func New(
	...
	m := &Manager{
		...
		assertionManager:             assertionManager,
		watcher:                      watcher,
		timeRef:                      clock.NewRealTimeReference(),
		trackedEdgeIds:               threadsafe.NewMap(threadsafe.MapWithMetric[protocol.EdgeId, *tracker.Tracker]("trackedEdgeIds")),
		assertionMetadataCache:       threadsafe.NewLruMap(maxAssertions, threadsafe.LruMapWithMetric[protocol.AssertionHash, state.AssociatedAssertionMetadata]("batchIndexForAssertionCache")),
...
func (m *Manager) MarkTrackedEdge(edgeId protocol.EdgeId, tracker *tracker.Tracker) {
...
func (m *Manager) getTrackerForEdge(ctx context.Context, edge protocol.VerifiedRoyalEdge) (*tracker.Tracker, error) {
	...
	var edgeTrackerAssertionInfo state.AssociatedAssertionMetadata
	if !ok {
		...
		edgeTrackerAssertionInfo = state.AssociatedAssertionMetadata{
			FromState:            fromState,
	...
	return retry.UntilSucceeds(ctx, func() (*tracker.Tracker, error) {
		return tracker.New(
			ctx,
			edge,
			m.chain,
			...
			tracker.WithTimeReference(m.timeRef),
			tracker.WithValidatorName(m.name),
		)
	})
}
