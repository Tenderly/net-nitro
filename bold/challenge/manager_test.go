...
func TestEdgeTracker_Act(t *testing.T) {
	...
	err = tkr.Act(ctx)
	require.NoError(t, err)
	require.Equal(t, tracker.EdgeAwaitingChallengeCompletion, tkr.CurrentState())
...
func TestEdgeTracker_Act_ConfirmedByTime(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, tracker.EdgeAwaitingChallengeCompletion, honestTracker.CurrentState())
...
func Test_getEdgeTrackers(t *testing.T) {
	...
	require.Equal(t, state.Batch(1), state.Batch(trk.AssertionInfo().FromState.Batch))
	require.Equal(t, state.Batch(100), trk.AssertionInfo().BatchLimit)
}
...
func setupEdgeTrackersForBisection(
	...
	tracker1, err := tracker.New(
		...
		honestValidator,
		assertionInfo,
		tracker.WithTimeReference(clock.NewArtificialTimeReference()),
		tracker.WithValidatorName(honestValidator.name),
	)
	require.NoError(t, err)
	...
	tracker2, err := tracker.New(
		...
		assertionInfo,
		tracker.WithTimeReference(clock.NewArtificialTimeReference()),
		tracker.WithValidatorName(evilValidator.name),
	)
	require.NoError(t, err)
