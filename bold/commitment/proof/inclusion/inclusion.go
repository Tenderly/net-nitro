...
// Package inclusion defines a series of utilities for generating and
...
func FullTree(leaves []common.Hash) ([][]common.Hash, error) {
	msb, err := prefix.MostSignificantBit(uint64(len(leaves)))
	if err != nil {
		return nil, err
	}
	lsb, err := prefix.LeastSignificantBit(uint64(len(leaves)))
	if err != nil {
		return nil, err
	}
	maxLevel := msb + 1
...
func GenerateInclusionProof(leaves []common.Hash, idx uint64) ([]common.Hash, error) {
	...
	maxLevel, err := prefix.MostSignificantBit(uint64(len(rehashed)) - 1)
	if err != nil {
		return nil, err
	}
