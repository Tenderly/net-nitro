...
import (
	...
	"github.com/tenderly/net-nitro/go-ethereum/ethclient/simulated"
...
func TestAppendCompleteSubTree(t *testing.T) {
	...
	_, err := prefix.AppendCompleteSubTree([]common.Hash{{1}}, prefix.MAX_LEVEL, common.Hash{2})
	require.ErrorContains(t, err, "level too high")

	// Test case: Empty Subtree Root
	_, err = prefix.AppendCompleteSubTree([]common.Hash{{1}}, 1, common.Hash{})
	require.ErrorContains(t, err, "cannot append empty")

	// Test case: Expansion Too Large
	_, err = prefix.AppendCompleteSubTree(make([]common.Hash, prefix.MAX_LEVEL+1), 1, common.Hash{2})
	require.ErrorContains(t, err, "merkle expansion to large")

	// Test case: Empty 'me' Array
	_, err = prefix.AppendCompleteSubTree([]common.Hash{}, 1, common.Hash{2})
	require.NoError(t, err)

	// Test case: Level >= len(me)
	_, err = prefix.AppendCompleteSubTree([]common.Hash{{1}}, 2, common.Hash{2})
...
func TestGeneratePrefixProof(t *testing.T) {
	...
	_, err := prefix.GeneratePrefixProof(0, nil, defaultLeaves, nil)
	require.ErrorContains(t, err, "prefixHeight was 0")

	// Test case: Zero Length of Leaves
	_, err = prefix.GeneratePrefixProof(1, nil, []common.Hash{}, nil)
...
func TestRoot(t *testing.T) {
	t.Run("tree with exactly size MAX_LEVEL should pass validation", func(t *testing.T) {
		tree := make([]common.Hash, prefix.MAX_LEVEL)
		_, err := prefix.Root(tree)
		require.NotEqual(t, prefix.ErrLevelTooHigh, err)
	})
	t.Run("tree too large", func(t *testing.T) {
		tree := make([]common.Hash, prefix.MAX_LEVEL+1)
		_, err := prefix.Root(tree)
		require.Equal(t, prefix.ErrExpansionTooLarge, err)
	})
	t.Run("empty tree", func(t *testing.T) {
		tree := make([]common.Hash, 0)
		_, err := prefix.Root(tree)
		require.Equal(t, prefix.ErrRootForEmpty, err)
	})
	t.Run("single element returns itself", func(t *testing.T) {
		tree := make([]common.Hash, 1)
		tree[0] = common.HexToHash("0x1234")
		root, err := prefix.Root(tree)
		require.NoError(t, err)
...
func TestVerifyPrefixProof_GoSolidityEquivalence(t *testing.T) {
	...
	fromMessageNumber := state.Height(3)
...
func TestVerifyPrefixProofWithHeight7_GoSolidityEquivalence1(t *testing.T) {
	...
	startMessageNumber := state.Height(0)
	fromMessageNumber := state.Height(3)
	toMessageNumber := state.Height(6)
	req := &state.HistoryCommitmentRequest{
		AssertionMetadata: &state.AssociatedAssertionMetadata{
			WasmModuleRoot: wasmModuleRoot,
			FromState: protocol.GoGlobalState{
...
func TestLeastSignificantBit_GoSolidityEquivalence(t *testing.T) {
	merkleTreeContract, _ := setupMerkleTreeContract(t)
	runBitEquivalenceTest(t, merkleTreeContract.LeastSignificantBit, prefix.LeastSignificantBit)
}

func TestMostSignificantBit_GoSolidityEquivalence(t *testing.T) {
	merkleTreeContract, _ := setupMerkleTreeContract(t)
	runBitEquivalenceTest(t, merkleTreeContract.MostSignificantBit, prefix.MostSignificantBit)
}
...
func FuzzPrefixProof_Verify(f *testing.F) {
	...
	testcases := []prefix.VerifyPrefixProofConfig{
		{
			PreRoot:  loCommit.Merkle,
			PreSize:  4,
			PostRoot: hiCommit.Merkle,
	...
	f.Fuzz(func(
		...
		cfg := &prefix.VerifyPrefixProofConfig{
		...
		goErr := prefix.VerifyPrefixProof(cfg)
...
func FuzzPrefixProof_MaximumAppendBetween_GoSolidityEquivalence(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, pre, post uint64) {
		gotGo, err1 := prefix.MaximumAppendBetween(pre, post)
...
func FuzzPrefixProof_BitUtils_GoSolidityEquivalence(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, x uint64) {
		...
		lsbGo, _ := prefix.LeastSignificantBit(x)
		if lsbSol != nil {
			if !lsbSol.IsUint64() {
		...
		msbGo, _ := prefix.MostSignificantBit(x)
		if msbSol != nil {
			if !msbSol.IsUint64() {
...
func TestVerifyPrefixProof(t *testing.T) {
	...
	t.Run("TestPreSizeZero", func(t *testing.T) {
		...
		err := prefix.VerifyPrefixProof(cfg)
	...
	t.Run("TestTreeSizeMismatch", func(t *testing.T) {
		r, err := prefix.Root([]common.Hash{{1}, {2}})
		require.NoError(t, err)
		cfg := &prefix.VerifyPrefixProofConfig{
			PreRoot:      r,
			PreSize:      1,
