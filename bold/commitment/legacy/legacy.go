...
func NewLegacy(leaves []common.Hash) (LegacyHistory, error) {
	...
	go func() {
		...
		firstLeafProof, err1 = inclusion.GenerateInclusionProof(leaves, 0)
	}()
	...
	go func() {
		defer waitGroup.Done()
		lastLeafProof, err2 = inclusion.GenerateInclusionProof(leaves, uint64(len(leaves))-1)
	}()
	...
	go func() {
		defer waitGroup.Done()
		exp := prefix.NewEmptyMerkleExpansion()
		for _, r := range leaves {
			exp, err3 = prefix.AppendLeaf(exp, r)
			if err3 != nil {
				return
			}
		}
		root, err3 = prefix.Root(exp)
	}()
	waitGroup.Wait()
