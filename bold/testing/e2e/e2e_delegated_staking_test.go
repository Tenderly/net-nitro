...
import (
	...
	"github.com/tenderly/net-nitro/bold/challenge"
...
func TestEndToEnd_DelegatedStaking(t *testing.T) {
	...
	baseStackOpts := []challenge.StackOpt{
		challenge.StackWithMode(types.MakeMode),
		challenge.StackWithPollingInterval(timeCfg.assertionScanningInterval),
		challenge.StackWithPostingInterval(timeCfg.assertionPostingInterval),
		challenge.StackWithAverageBlockCreationTime(timeCfg.blockTime),
		challenge.StackWithConfirmationInterval(timeCfg.assertionConfirmationAttemptInterval),
		challenge.StackWithMinimumGapToParentAssertion(0),
		challenge.StackWithHeaderProvider(shp),
		challenge.StackWithDelegatedStaking(), // Enable delegated staking.
		challenge.StackWithoutAutoDeposit(),
	}
	...
	honestChain := setupAssertionChain(
		...
		rollupAddr.Rollup,
		honestTxOpts,
		sol.WithCustomWithdrawalAddress(fundsCustodianOpts.From),
	)
	...
	evilChain := setupAssertionChain(
		...
		rollupAddr.Rollup,
		evilTxOpts,
		sol.WithCustomWithdrawalAddress(evilFundsCustodianOpts.From),
	)
	...
	honestManager, err := challenge.NewChallengeStack(honestChain, honestStateManager, honestOpts...)
	require.NoError(t, err)
	_ = honestManager

	evilManager, err := challenge.NewChallengeStack(evilChain, evilStateManager, evilOpts...)
	require.NoError(t, err)
	_ = evilManager
