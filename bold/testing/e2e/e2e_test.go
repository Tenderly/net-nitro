...
func runEndToEndTest(t *testing.T, cfg *e2eConfig) {
	...
	baseStackOpts := []challenge.StackOpt{
		...
		challenge.StackWithPollingInterval(cfg.timings.assertionScanningInterval),
		challenge.StackWithPostingInterval(cfg.timings.assertionPostingInterval),
		challenge.StackWithAverageBlockCreationTime(cfg.timings.blockTime),
		challenge.StackWithConfirmationInterval(cfg.timings.assertionConfirmationAttemptInterval),
	...
	evilChallengeManagers := make([]*challenge.Manager, cfg.actors.numEvilValidators)
