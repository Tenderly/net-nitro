...
import (
	...
	"github.com/tenderly/net-nitro/bold/challenge-manager/challenge-tree/mock"
...
func Test_localTimer(t *testing.T) {
	...
	t.Run("zero if earlier than creation time", func(t *testing.T) {
		timer, err := ct.LocalTimer(ctx, edgeA, edgeA.CreationBlock-1)
		require.NoError(t, err)
		require.Equal(t, uint64(0), timer)
	})
	t.Run("no rival is simply difference between T and creation time", func(t *testing.T) {
		timer, err := ct.LocalTimer(ctx, edgeA, edgeA.CreationBlock)
		require.NoError(t, err)
		require.Equal(t, uint64(0), timer)
		timer, err = ct.LocalTimer(ctx, edgeA, edgeA.CreationBlock+3)
		require.NoError(t, err)
		require.Equal(t, uint64(3), timer)
		timer, err = ct.LocalTimer(ctx, edgeA, edgeA.CreationBlock+1000)
		require.NoError(t, err)
